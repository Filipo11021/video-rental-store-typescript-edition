...
import { identity } from './function/function.ts';
import { NanoIdLibDep } from './nano-id/nano-id.ts';
import { isPlainObject } from './object/object.ts';
import { Err, err, Ok, ok, Result, trySync } from '@repo/type-safe-errors';
import { safelyStringifyUnknownValue } from './string/string.ts';
import type { Brand, Literal, Simplify, WidenLiteral } from './types/types.ts';
import { IntentionalNever } from './types/types.ts';
...
export type InferParentError<A extends AnyType> =
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  A extends Type<any, any, any, any, any, infer ParentError> ? ParentError : never;

export type InferErrors<T extends AnyType> =
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  T extends Type<any, any, any, infer Error, any, infer ParentError> ? Error | ParentError : never;
...
>(
  ...
  definition: Omit<
    ...
    'name' | 'is' | 'from' | typeof EvoluTypeSymbol | 'Type' | 'Input' | 'Error' | 'Parent' | 'ParentError' | 'Errors'
  >,
): Type<Name, T, Input, Error, Parent, ParentError> => ({
  ...definition,
  name,
...
export const createTypeErrorFormatter =
  <Error extends TypeError>(
    format: (error: Simplify<Omit<Error, 'value'> & { value: string }>) => string,
 ...
 * @category Utilities
 */
export const createBaseTypeErrorFormatter = <Error extends TypeError>(): TypeErrorFormatter<Error> =>
  createTypeErrorFormatter<Error>((error) => `A value ${error.value} is not a ${error.type.toLowerCase()}.`);
...
export const String = base('String', (value) =>
  typeof value === 'string' ? ok(value) : err<StringError>({ type: 'String', value }),
);

export type StringError = TypeError<'String'>;

export const formatStringError = createBaseTypeErrorFormatter<StringError>();

/** @category Base Types */
export const Number = base('Number', (value) =>
  typeof value === 'number' ? ok(value) : err<NumberError>({ type: 'Number', value }),
);

export type NumberError = TypeError<'Number'>;

export const formatNumberError = createBaseTypeErrorFormatter<NumberError>();

/** @category Base Types */
export const BigInt = base('BigInt', (value) =>
  typeof value === 'bigint' ? ok(value) : err<BigIntError>({ type: 'BigInt', value }),
);

export type BigIntError = TypeError<'BigInt'>;

export const formatBigIntError = createBaseTypeErrorFormatter<BigIntError>();

/** @category Base Types */
export const Boolean = base('Boolean', (value) =>
  typeof value === 'boolean' ? ok(value) : err<BooleanError>({ type: 'Boolean', value }),
);

export type BooleanError = TypeError<'Boolean'>;

export const formatBooleanError = createBaseTypeErrorFormatter<BooleanError>();

/** @category Base Types */
export const Undefined = base('Undefined', (value) =>
  value === undefined ? ok(value) : err<UndefinedError>({ type: 'Undefined', value }),
);

export type UndefinedError = TypeError<'Undefined'>;

export const formatUndefinedError = createBaseTypeErrorFormatter<UndefinedError>();

/** @category Base Types */
export const Null = base('Null', (value) => (value === null ? ok(value) : err<NullError>({ type: 'Null', value })));

export type NullError = TypeError<'Null'>;

export const formatNullError = createBaseTypeErrorFormatter<NullError>();

/** @category Base Types */
export const Function = base('Function', (value) =>
  typeof value === 'function' ? ok(value) : err<FunctionError>({ type: 'Function', value }),
);

export type FunctionError = TypeError<'Function'>;

export const formatFunctionError = createBaseTypeErrorFormatter<FunctionError>();

/** @category Base Types */
export const Uint8Array = base('Uint8Array', (value) =>
  value instanceof globalThis.Uint8Array ? ok(value) : err<Uint8ArrayError>({ type: 'Uint8Array', value }),
);

export type Uint8ArrayError = TypeError<'Uint8Array'>;

export const formatUint8ArrayError = createBaseTypeErrorFormatter<Uint8ArrayError>();
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const instanceOf = <T extends abstract new (...args: any) => any>(ctor: T): InstanceOfType<T> => ({
  ...base('InstanceOf', (value) =>
    value instanceof ctor ? ok(value) : err<InstanceOfError>({ type: 'InstanceOf', value, ctor: ctor.name }),
  ),
  ctor,
});
...
export interface InstanceOfType<T extends abstract new (...args: any) => any>
  extends Type<'InstanceOf', InstanceType<T>, InstanceType<T>, InstanceOfError> {
  ctor: T;
}
...
export type EvoluTypeError = TypeError<'EvoluType'>;
...
export type CurrencyCodeError = TypeError<'CurrencyCode'>;

export const formatCurrencyCodeError = createTypeErrorFormatter<CurrencyCodeError>(
  (error) => `Invalid currency code: ${error.value}`,
);
...
export type DateIsoStringError = TypeError<'DateIsoString'>;

export const formatDateIsoStringError = createTypeErrorFormatter<DateIsoStringError>(
 ...
 * @category Utilities
 */
export type BrandFactory<Name extends TypeName, Input, RefineError extends TypeError> = <
 ...
 * @category String
 */
export const trimmed: BrandFactory<'Trimmed', string, TrimmedError> = (parent) =>
  brand('Trimmed', parent, (value) =>
    value.trim().length === value.length ? ok(value) : err<TrimmedError>({ type: 'Trimmed', value }),
  );

export type TrimmedError = TypeError<'Trimmed'>;

export const formatTrimmedError = createTypeErrorFormatter<TrimmedError>(
  (error) => `A value ${error.value} is not trimmed`,
);

export type TransformBrandFactory<Name extends TypeName, Input, TransformError extends TypeError = never> = <
  PName extends TypeName,
  P extends Input,
  PInput,
  PParent,
...
>(
  ...
  BrandType<Type<PName, P, PInput, PError, PParent, PParentError>, Name, never, PError | PParentError>,
  TransformError
>;
...
export const formatMinLengthError = createTypeErrorFormatter<MinLengthError>(
  (error) => `Value ${error.value} does not meet the minimum length of ${error.min}.`,
);
...
export const maxLength: <Max extends number>(
  max: Max,
) => BrandFactory<`MaxLength${Max}`, { length: number }, MaxLengthError<Max>> = (max) => (parent) =>
...
export const length: <Exact extends number>(
  exact: Exact,
) => BrandFactory<`Length${Exact}`, { length: number }, LengthError<Exact>> = (exact) => (parent) =>
  brand(`Length${exact}`, parent, (value) =>
    value.length === exact ? ok(value) : err({ type: 'Length', value, exact }),
  );
...
export const formatLengthError = createTypeErrorFormatter<LengthError>(
  (error) => `Value ${error.value} does not have the required length of ${error.exact}.`,
);
...
export const Mnemonic = brand('Mnemonic', NonEmptyTrimmedString, (value) =>
  bip39.validateMnemonic(value, wordlist) ? ok(value) : err<MnemonicError>({ type: 'Mnemonic', value }),
);
export type Mnemonic = typeof Mnemonic.Type;

export type MnemonicError = TypeError<'Mnemonic'>;
...
export const formatRegexError = createTypeErrorFormatter<RegexError>(
  (error) => `Value ${error.value} does not match the pattern for ${error.name}: ${error.pattern}`,
);
...
export const formatSimplePasswordError = (
  formatTypeError: TypeErrorFormatter<StringError | MinLengthError<8> | MaxLengthError<64> | TrimmedError>,
): TypeErrorFormatter<SimplePasswordError> =>
  createTypeErrorFormatter<SimplePasswordError>((error) => `Invalid password: ${formatTypeError(error.parentError)}`);
 ...
 * ```
 */
export const createId = (deps: NanoIdLibDep): Id => deps.nanoIdLib.nanoid() as Id;
...
export interface IdType<Table extends TypeName>
  extends Type<'Id', string & Brand<'Id'> & Brand<Table>, string, IdError<Table>, string, StringError> {
  table: Table;
}
 ...
 * @category Number
 */
export const positive: BrandFactory<'Positive', number, PositiveError> = (parent) =>
  brand('Positive', parent, (value) => (value > 0 ? ok(value) : err<PositiveError>({ type: 'Positive', value })));

export type PositiveError = TypeError<'Positive'>;
 ...
 * @category Number
 */
export const negative: BrandFactory<'Negative', number, NegativeError> = (parent) =>
  brand('Negative', parent, (value) => (value < 0 ? ok(value) : err<NegativeError>({ type: 'Negative', value })));

export type NegativeError = TypeError<'Negative'>;
 ...
 * @category Number
 */
export const nonPositive: BrandFactory<'NonPositive', number, NonPositiveError> = (parent) =>
  brand('NonPositive', parent, (value) =>
    value <= 0 ? ok(value) : err<NonPositiveError>({ type: 'NonPositive', value }),
  );

export type NonPositiveError = TypeError<'NonPositive'>;

export const formatNonPositiveError = createTypeErrorFormatter<NonPositiveError>(
 ...
 * @category Number
 */
export const nonNegative: BrandFactory<'NonNegative', number, NonNegativeError> = (parent) =>
  brand('NonNegative', parent, (value) =>
    value >= 0 ? ok(value) : err<NonNegativeError>({ type: 'NonNegative', value }),
  );

export type NonNegativeError = TypeError<'NonNegative'>;

export const formatNonNegativeError = createTypeErrorFormatter<NonNegativeError>(
...
export const int: BrandFactory<'Int', number, IntError> = (parent) =>
  brand('Int', parent, (value) =>
    globalThis.Number.isSafeInteger(value) ? ok(value) : err<IntError>({ type: 'Int', value }),
  );

export type IntError = TypeError<'Int'>;
...
export const greaterThan: <Min extends number>(
  min: Min,
) => BrandFactory<`GreaterThan${Min}`, number, GreaterThanError<Min>> = (min) => (parent) =>
  brand(`GreaterThan${min}`, parent, (value) => (value > min ? ok(value) : err({ type: 'GreaterThan', value, min })));

export interface GreaterThanError<Min extends number = number> extends TypeError<'GreaterThan'> {
  readonly min: Min;
}

export const formatGreaterThanError = createTypeErrorFormatter<GreaterThanError>(
 ...
 * @category Number
 */
export const lessThan: <Max extends number>(max: Max) => BrandFactory<`LessThan${Max}`, number, LessThanError<Max>> =
  (max) => (parent) =>
    brand(`LessThan${max}`, parent, (value) => (value < max ? ok(value) : err({ type: 'LessThan', value, max })));
...
export const greaterThanOrEqualTo: <Min extends number>(
  min: Min,
) => BrandFactory<`GreaterThanOrEqualTo${Min}`, number, GreaterThanOrEqualToError<Min>> = (min) => (parent) =>
  brand(`GreaterThanOrEqualTo${min}`, parent, (value) =>
    value >= min ? ok(value) : err({ type: 'GreaterThanOrEqualTo', value, min }),
  );
...
export const formatGreaterThanOrEqualToError = createTypeErrorFormatter<GreaterThanOrEqualToError>(
  (error) => `The value ${error.value} is not >= ${error.min}.`,
);
...
export const lessThanOrEqualTo: <Max extends number>(
  max: Max,
) => BrandFactory<`LessThanOrEqualTo${Max}`, number, LessThanOrEqualToError<Max>> = (max) => (parent) =>
...
export const formatLessThanOrEqualToError = createTypeErrorFormatter<LessThanOrEqualToError>(
  (error) => `The value ${error.value} is not <= ${error.max}.`,
);
...
export const nonNaN: BrandFactory<'NonNaN', number, NonNaNError> = (parent) =>
  brand('NonNaN', parent, (value) =>
    !globalThis.Number.isNaN(value) ? ok(value) : err<NonNaNError>({ type: 'NonNaN', value }),
  );

export type NonNaNError = TypeError<'NonNaN'>;
...
export const finite: BrandFactory<'Finite', number, FiniteError> = (parent) =>
  brand('Finite', parent, (value) =>
    globalThis.Number.isFinite(value) ? ok(value) : err<FiniteError>({ type: 'Finite', value }),
  );

export type FiniteError = TypeError<'Finite'>;
...
export const multipleOf: <Divisor extends number>(
  divisor: Divisor,
) => BrandFactory<`MultipleOf${Divisor}`, number, MultipleOfError<Divisor>> = (divisor) => (parent) =>
  brand(`MultipleOf${divisor}`, parent, (value) =>
    value % divisor === 0 ? ok(value) : err({ type: 'MultipleOf', value, divisor }),
  );
...
export const between: <Min extends number, Max extends number>(
  min: Min,
  max: Max,
) => BrandFactory<`Between${Min}-${Max}`, number, BetweenError<Min, Max>> = (min, max) => (parent) =>
  brand(`Between${min}-${max}`, parent, (value) =>
    value >= min && value <= max ? ok(value) : err({ type: 'Between', value, min, max }),
  );
...
export const formatBetweenError = createTypeErrorFormatter<BetweenError>(
  (error) => `The value ${error.value} is not between ${error.min} and ${error.max}, inclusive.`,
);
...
export const literal = <T extends Literal>(expected: T): LiteralType<T> => {
  const fromUnknown = (value: unknown): Result<T, LiteralError<T>> =>
    value === expected ? ok(expected) : err<LiteralError<T>>({ type: 'Literal', value, expected });
...
export const formatLiteralError = createTypeErrorFormatter<LiteralError>(
  (error) =>
    `The value ${error.value} is not strictly equal to the expected literal: ${globalThis.String(error.expected)}.`,
);
 ...
 * @category Base Factories
 */
export const transform = <FromType extends AnyType, ToType extends AnyType, TransformError extends TypeError = never>(
  fromType: FromType,
  toType: ToType,
  fromParent: (parentValue: InferType<FromType>) => Result<InferType<ToType>, TransformError>,
  toParent: (value: InferType<ToType>) => InferType<FromType>,
): TransformType<FromType, ToType, TransformError> => {
  const fromUnknown = (value: unknown): Result<InferType<ToType>, InferErrors<FromType> | TransformError> => {
    const parentResult = fromType.fromUnknown(value);
    if (!parentResult.ok) return parentResult;
    return fromParent(parentResult.value as IntentionalNever);
  };

  const to = (value: InferType<ToType>): InferInput<FromType> => fromType.to(toParent(value)) as IntentionalNever;
...
> extends Type<
  ...
  ) => [TransformError] extends [never] ? Ok<InferType<ToType>> : Result<InferType<ToType>, TransformError>;
}
...
export const NumberFromString = transform(
  ...
  (value) => {
    ...
    if (!result.ok) return err<NumberFromStringError>({ type: 'NumberFromString', value });
    return result;
  },
  (num) => num.toString() as NonEmptyTrimmedString,
);

export type NumberFromStringError = TypeError<'NumberFromString'>;

export const formatNumberFromStringError = createTypeErrorFormatter<NumberFromStringError>((error) => {
 ...
 * @category Array
 */
export const array = <ElementType extends AnyType>(element: ElementType): ArrayType<ElementType> => {
  const fromUnknown = (
    value: unknown,
  ): Result<ReadonlyArray<InferType<ElementType>>, ArrayError<InferErrors<ElementType>>> => {
  ...
  ): Result<ReadonlyArray<InferType<ElementType>>, ArrayError<InferError<ElementType>>> => {
    const result: Array<InferType<ElementType>> = [];
    for (let i = 0; i < value.length; i++) {
...
export type ArrayError<Error extends TypeError = TypeError> = TypeErrorWithReason<
  'Array',
...
>(
  ...
  const fromUnknown = (
    value: unknown,
  ): Result<Record<KeyT, InferType<Value>>, RecordError<KeyError, InferError<Value>>> => {
    ...
    const result: Record<KeyT, InferType<Value>> = {} as Record<KeyT, InferType<Value>>;
  ...
  const to = (value: Readonly<Record<KeyT, InferType<Value>>>): Readonly<Record<KeyInput, InferInput<Value>>> =>
    Object.fromEntries(
      Object.entries(value).map(([key, val]) => [keyType.to(key as KeyT), valueType.to(val)]),
  ...
  const toParent = (value: Readonly<Record<KeyT, InferType<Value>>>): Readonly<Record<KeyParent, InferParent<Value>>> =>
    Object.fromEntries(
      Object.entries(value).map(([key, val]) => [keyType.toParent(key as KeyT), valueType.toParent(val)]),
 ...
 * @category Object
 */
export function object<Props extends Record<string, AnyType>>(props: Props): ObjectType<Props>;
...
export function object(
  ...
  const fromUnknown = (
    value: unknown,
  ): Result<any, ObjectError<Record<string, any>> | ObjectWithRecordError<any, any, any>> => {
    if (!isPlainObject(value)) {
      return err({
...
export type ObjectError<PropsErrors extends Record<string, TypeError> = Record<string, TypeError>> =
  TypeErrorWithReason<
    'Object',
...
export type MergeObjectTypeErrors<T extends ObjectType<any>> =
  T extends ObjectType<infer Props> ? ObjectError<{ [K in keyof Props]: InferErrors<Props[K]> }> : never;
...
> extends Type<
    ...
    ObjectWithRecordError<{ [K in keyof Props]: InferError<Props[K]> }, KeyError, InferError<Value>>,
    Readonly<ObjectParent<Props>> & Readonly<Record<KeyParent, InferParent<Value>>>,
    ObjectWithRecordError<{ [K in keyof Props]: InferParentError<Props[K]> }, KeyParentError, InferParentError<Value>>
  > {
  readonly props: Props;
  readonly record: RecordType<KeyName, KeyT, KeyInput, KeyError, KeyParent, KeyParentError, Value>;
}

export type ObjectWithRecordError<
 ...
 * @category Base Factories
 */
export function union<Members extends [AnyType, AnyType, ...ReadonlyArray<AnyType>]>(
  ...members: Members
): UnionType<Members>;

export function union<Literals extends [Literal, Literal, ...ReadonlyArray<Literal>]>(
  ...literals: Literals
): UnionType<{
...
export const isUnionType = (t: AnyType): t is UnionType<[AnyType, ...ReadonlyArray<AnyType>]> =>
 ...
 * @category Base Factories
 */
export const recursive = <ParentType extends AnyType>(create: () => ParentType): RecursiveType<ParentType> => {
 ...
 * @category Base Factories
 */
export const nullOr = <T extends AnyType>(type: T): UnionType<[typeof Null, T]> => union(Null, type);
 ...
 * @category Base Factories
 */
export const undefinedOr = <T extends AnyType>(type: T): UnionType<[typeof Undefined, T]> => union(Undefined, type);
 ...
 * @category Base Factories
 */
export const nullishOr = <T extends AnyType>(type: T): UnionType<[typeof Undefined, typeof Null, T]> =>
...
export const tuple = <Elements extends [AnyType, ...ReadonlyArray<AnyType>]>(
  ...
  const fromUnknown = (
    value: unknown,
  ): Result<{ [K in keyof Elements]: InferType<Elements[K]> }, TupleError<InferError<Elements[number]>>> => {
  ...
  const toParent = (value: { [K in keyof Elements]: InferType<Elements[K]> }): {
    ...
    value.map((val, index) => elements[index].toParent(val) as IntentionalNever) as {
      [K in keyof Elements]: InferParent<Elements[K]>;
    };
 ...
 * reflection.
 */
export interface TupleType<Elements extends readonly [AnyType, ...ReadonlyArray<AnyType>]>
  extends Type<
    'Tuple',
...
export type TupleError<ElementError extends TypeError = TypeError> = TypeErrorWithReason<
  'Tuple',
...
export type Int64Error = TypeError<'Int64'>;

export const formatInt64Error = createTypeErrorFormatter<Int64Error>(
  (error) => `The value ${error.value} is not a valid 64-bit signed integer (Int64).`,
);
...
export type BigIntFromStringError = TypeError<'BigIntFromString'>;

export const formatBigIntFromStringError = createTypeErrorFormatter<BigIntFromStringError>(
...
export type Int64StringError = TypeError<'Int64String'>;

export const formatInt64StringError = createTypeErrorFormatter<Int64StringError>(
...
export const JsonValue = recursive(
  (): UnionType<
    [
      ...
      typeof Boolean,
      typeof Null,
      ArrayType<Type<'Recursive', JsonValue, JsonValueInput, JsonValueError, JsonValueInput, JsonValueError>>,
      RecordType<
        'String',
        string,
        string,
        ...
        string,
        StringError,
        Type<'Recursive', JsonValue, JsonValueInput, JsonValueError, JsonValueInput, JsonValueError>
      >,
    ]
...
export const formatJsonValueFromStringError = createTypeErrorFormatter<JsonValueFromStringError>(
...
export interface OptionalType<T extends AnyType>
  extends Type<'Optional', InferType<T>, InferInput<T>, InferError<T>, InferParent<T>, InferParentError<T>> {
  readonly parent: T;
}
...
export type NullTypeInMembers<Members extends [AnyType, ...Array<AnyType>]> = Members extends [
  infer Head,
  ...infer Tail,
]
...
export const json = <T extends AnyType, Name extends TypeName>(
  type: T,
  name: Name,
): TransformType<T, BrandType<typeof String, Name, JsonValueFromStringError | T['Errors'], StringError>> => {
  type E = JsonValueFromStringError | T['Errors'];

  const BrandedJsonString: BrandType<typeof String, Name, E, StringError> = brand(name, String, (value) => {
...
export const createFormatTypeError = <ExtraErrors extends TypeError = never>(
  ...
  //@ts-expect-error - this is a recursive type
  const formatTypeError: TypeErrorFormatter<TypeErrors<ExtraErrors>> = (error) => {
