...
import { err, getOrThrow, ok, Result, tryAsync, trySync } from './index.ts';
...
test('getOrThrow', () => {
  ...
  expect(() => getOrThrow(err('error'))).toThrowErrorMatchingInlineSnapshot(`[Error: Result error: "error"]`);
});
...
test.skip('Result wrapping vs unwrapped performance', () => {
  ...
  const readWrapped = (bytes: Uint8Array, offset: number, size: number): Result<Uint8Array, string> => {
    return ok(bytes.subarray(offset, offset + size));
  };

  // Unwrapped: Read without Result
  const readUnwrapped = (bytes: Uint8Array, offset: number, size: number): Uint8Array => {
  ...
  console.log(`Unwrapped: ${unwrappedTime.toFixed(2)} ms for ${NUM_ITEMS} items`);
